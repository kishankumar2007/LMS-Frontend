...
const Navbar = () => {
    ...
    const NavItems = [{ name: 'Home', path: "/" }, { name: "Categories", path: "/categories" }, { name: 'All Courses', path: "/courses" }, {name:"All Teachers",path:"/teachers"}]
    ...
    return (
        <div className={`sm:px-20  px-6 flex items-center justify-between w-full ${isScrolled ? "bg-white/5" : "bg-transparent"}  sm:h-16 h-14 drop-shadow backdrop-blur-xs fixed transition-all ease-in duration-300 left-0 right-0 top-0 z-99`}>
            ...
            <button onClick={() => navigate("/login")} className='text-zinc-300 flex gap-2 bg-linear-to-r active:scale-95 hover:scale-105 transition-all ease-in duration-150 from-cyan-400 font-base drop-shadow to-purple-700 px-6 py-2 rounded-full items-center'>Login
